...
public class Interpreter {
	...
	public void runExe()
	{
		...
		for(int c = 0; c < exe.length; c++)
		{
			...
			if(s.matches("(clear )([a-z]|[A-Z]|[0-9])*"))
			{
				if(getIndex(s) == -1)
				{
			...
			if(s.matches("(incr )([a-z]|[A-Z]|[0-9])*"))
			{
				var.get(getIndex(s)).incVal();
				printVar();
			}
			
			if(s.matches("(decr )([a-z]|[A-Z]|[0-9])*"))
			{
			...
			if(s.matches("(while )([a-z]|[A-Z]|[0-9])*( not 0 do)"))
			{
	...
	public void wLoop(int c)
	{
		...
		{
			String s1 = exe[c1];
			if(s1.matches("(while )([a-z]|[A-Z]|[0-9])*( not 0 do)"))
			{
				count1 += 1;
			}
	...
	public void playSound(){
		//"C:/Users/Chris/Documents/Lightspeedshortened.wav"		        
		        try
		        {
		            Clip clip = AudioSystem.getClip();
		            clip.open(AudioSystem.getAudioInputStream(new File("C:/Users/Chris/Documents/Lightspeedshortened.wav")));
		            clip.start();
